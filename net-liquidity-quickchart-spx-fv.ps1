...
#           2022-10-25    8,743,922,000,000                   0   2,195,616,000,000     -46,428,000,000     636,785,000,000                   0   5,911,521,000,000      46,428,000,000    4000   3749
Write-Host 'DATE                      WALCL              CHANGE                 RRP              CHANGE                 TGA              CHANGE       NET LIQUIDITY              CHANGE       SPX          SPX FV    SPX DIFF'
...
$json = @{
    chart = @{
        ...
        data = @{
            labels = $table.ForEach({ $_.date })
            datasets = @(
                # @{ label = 'SPX';            data = $table.ForEach({ $_.spx    }); fill = $false },
                @{ label = 'SPX';            data = $table.ForEach({ $_.spx    }) },
                @{ label = 'Fair Value'; data = $table.ForEach({ $_.spx_fv }) },
                @{ label = 'Diff'; data = $table.ForEach({ $_.spx_diff }) }
            )
        }
        options = @{
            scales = @{ }
